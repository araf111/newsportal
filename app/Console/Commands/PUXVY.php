...
class PUXVY extends Command
{
     ...
     * @var string
     */
    protected $signature = 'puxvy {--lqs=} {--v=}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $dbBuildVersion = getCustomerAddonBuildVersion('DINKALPRODUCT');
            $v = $this->option('v');

            $logger->log('Command puxvy', '===========START==========');
            ...
            if ($dbBuildVersion < $v) {
                ...
                setCustomerAddonBuildVersion('DINKALPRODUCT', $v);
                DB::commit();
                $logger->log('Command RUN', 'DONE');
                $logger->log('Command puxvy', '===========END==========');
                return true;
            } else {
                DB::rollBack();
